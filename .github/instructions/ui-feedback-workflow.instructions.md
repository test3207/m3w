...
- The canonical store lives in `src/components/ui/use-toast.ts`; do not create per-feature stores.
...
- Mount exactly one `<Toaster />` component from `src/components/ui/toaster.tsx` in `src/app/layout.tsx`.
- Do not mount additional toaster instances inside feature routes; they will cause duplicate toasts.

## Triggering Toasts

- Server Actions should return `{ status, message }` payloads. Client components map `status === "success"` to `toast.success` and everything else to `toast.error`.
...
- Use descriptive `message` strings sourced from locale files under `src/locales` to keep translations centralized.

## Testing

- Cover toast store behavior with unit tests in `src/test/ui/toast.test.ts` (create or extend as needed).
