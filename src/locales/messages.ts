export const ERROR_MESSAGES = {
  unauthorized: "Unauthorized",
  noFileProvided: "No file provided",
  libraryIdRequired: "libraryId is required",
  invalidFileType: "Invalid file type. Only audio files are allowed.",
  fileTooLarge: "File too large. Maximum size is 100MB.",
  fileUploadFailed: "File upload failed. Please try again.",
  invalidInput: "Invalid input",
  failedToRetrieveLibraries: "Failed to retrieve libraries",
  failedToRetrieveLibrary: "Failed to retrieve library",
  failedToCreateLibrary: "Failed to create library",
  libraryNotFound: "Library not found",
  failedToUpdateLibrary: "Failed to update library",
  failedToDeleteLibrary: "Failed to delete library",
  failedToGetPlaylist: "Failed to retrieve playlist",
  failedToGetPlaylists: "Failed to retrieve playlists",
  failedToCreatePlaylist: "Failed to create playlist",
  failedToAddSongToPlaylist: "Failed to add song to playlist",
  failedToRemoveSongFromPlaylist: "Failed to remove song from playlist",
  playlistOrSongNotFound: "Playlist or song not found",
  failedToDeletePlaylist: "Failed to delete playlist",
  playlistNotFound: "Playlist not found",
  invalidSongOrder: "Invalid song order",
  failedToReorderSongs: "Failed to reorder playlist songs",
  uploadFailed: "上传失败，请稍后再试",
  uploadErrorGeneric: "上传过程中出现问题，请稍后再试",
  noAudioFileSelected: "请选择一个音频文件",
  noLibrarySelected: "请选择要保存的音乐库",
  libraryUnavailable: "Library not found",
} as const;

export const SUCCESS_MESSAGES = {
  uploadCompleted: "File upload completed",
} as const;

export const UI_TEXT = {
  dashboard: {
    title: "M3W Dashboard",
    welcomePrefix: "Welcome back, ",
    welcomeFallback: "there",
    welcomeSuffix: "! 👋",
    welcomeDescription: "Your Next.js full-stack application is ready. Start building amazing features!",
    badgeProductionReady: "Production Ready",
    librariesCard: {
      title: "🎵",
      titleSuffix: "Your Libraries",
      description: "Manage your personal music collections",
      empty: "You have not created any libraries yet.",
      songsSuffix: " songs",
    },
    playlistsCard: {
      title: "📻",
      titleSuffix: "Your Playlists",
      description: "Curate tracks across libraries and reorder them anytime",
      empty: "No playlists yet. Create one to start organizing tracks.",
      songsSuffix: " songs",
    },
    gettingStartedCard: {
      title: "🧭",
      titleSuffix: "Getting Started",
      description: "Recommended actions for new accounts",
      createLibraryTitle: "Create a library",
      createLibraryDescription: "Upload tracks and keep them organized by genre or mood.",
      createLibraryButton: "Go to Library Manager",
      buildPlaylistTitle: "Build a playlist",
      buildPlaylistDescription: "Combine songs from different libraries into a single mix.",
      buildPlaylistButton: "Open Playlist Builder",
      offlineTitle: "Save for offline",
      offlineDescription: "Choose playlists to cache locally for uninterrupted playback.",
      offlineButton: "Configure Offline Storage",
    },
    storageCard: {
      title: "📊",
      titleSuffix: "Storage At A Glance",
      description: "Keep track of how much data you sync locally",
      placeholder: "Storage metrics will appear once offline caching is configured.",
    },
    quickActions: {
      upload: "Upload Songs",
      manageLibraries: "Manage Libraries",
      buildPlaylists: "Build Playlists",
    },
    navbar: {
      title: "M3W Dashboard",
      signOut: "Sign Out",
    },
  },
  libraryManager: {
    pageTitle: "Library Manager",
    pageDescription: "Create, review, and remove music libraries. Each library keeps its own metadata and songs.",
    createCardTitle: "Create a new library",
    nameLabel: "Library name",
    namePlaceholder: "Jazz Collection",
    descriptionLabel: "Description",
    descriptionPlaceholder: "Optional notes about this library",
    createButton: "Create library",
    listTitle: "Your libraries",
    empty: "No libraries yet. Use the form to create your first collection.",
    songsSuffix: " songs",
    createdOnPrefix: "Created on ",
    updatedOnPrefix: "Updated ",
    deleteButton: "Delete",
    deleteWarning: "目前还没有可用的音乐库，请先前往 Library Manager 创建一个库。",
    detailTitlePrefix: "Library: ",
    detailDescription: "Manage songs in this library and add them to your playlists.",
    backToLibraries: "Back to libraries",
    songListTitle: "Songs in this library",
    songListEmpty: "No songs found in this library yet.",
    songCountLabel: "Songs",
    songAlbumLabel: "Album",
    songDurationLabel: "Duration",
    addToPlaylistLabel: "Add to playlist",
    addToPlaylistPlaceholder: "Select a playlist",
    addToPlaylistButton: "Add",
    addingToPlaylistLabel: "Adding...",
    toastAddSuccessTitle: "Added to playlist",
    toastAddSuccessDescription: "Song added successfully.",
    toastAddErrorTitle: "Unable to add song",
    toastAddErrorDescription: "Please try again.",
    selectPlaylistFirst: "Select a playlist first.",
    noPlaylistsHelper: "Create a playlist first to start adding songs.",
    goToPlaylistsLink: "Go to playlists",
  },
  playlistBuilder: {
    pageTitle: "Playlist Builder",
    pageDescription: "Combine songs across libraries, set custom artwork, and keep your favorite mixes up to date.",
    createCardTitle: "Create a new playlist",
    nameLabel: "Playlist name",
    namePlaceholder: "Road Trip Mix",
    descriptionLabel: "Description",
    descriptionPlaceholder: "Capture the mood or story behind this playlist",
    coverLabel: "Cover image URL",
    coverPlaceholder: "https://example.com/cover.jpg",
    createButton: "Create playlist",
    listTitle: "Your playlists",
    empty: "No playlists yet. Use the form to build your first mix.",
    songsSuffix: " songs",
    coverPrefix: "Cover: ",
    createdOnPrefix: "Created on ",
    updatedOnPrefix: "Updated ",
    deleteButton: "Delete",
    detailTitlePrefix: "Playlist: ",
    detailDescription: "Review songs in this playlist, adjust ordering, and remove tracks you no longer need.",
    backToPlaylists: "Back to playlists",
    songListTitle: "Songs in this playlist",
    songListEmpty: "No songs in this playlist yet.",
    addedFromLibraryHelper: "Add songs from your libraries to build this playlist.",
    songCountLabel: "Songs",
    songAlbumLabel: "Album",
    songDurationLabel: "Duration",
    removeButton: "Remove",
    moveUp: "Move up",
    moveDown: "Move down",
    playlistDurationLabel: "Total duration",
    manageLibrariesCta: "Manage libraries",
    toastMoveSuccessTitle: "Order updated",
    toastMoveUpDescriptionPrefix: "Moved up: ",
    toastMoveDownDescriptionPrefix: "Moved down: ",
    toastRemoveSuccessTitle: "Song removed",
    toastRemoveSuccessDescriptionPrefix: "Removed: ",
    toastActionErrorTitle: "Action failed",
    toastActionErrorDescription: "Please try again.",
  },
  uploadPage: {
    title: "上传歌曲",
    description: "将音频文件保存到自己的音乐库中，系统会自动处理文件去重并提取基础信息。",
    cardTitle: "上传音频文件",
    emptyState: "目前还没有可用的音乐库，请先前往 Library Manager 创建一个库。",
  },
  uploadForm: {
    selectLibraryLabel: "选择音乐库",
    selectLibraryPlaceholder: "上传的歌曲会保存到所选音乐库中。",
    libraryOptionFallback: "请先创建一个音乐库",
    librarySongSuffix: " 首歌曲",
    selectFileLabel: "选择音频文件",
    fileHelper: "支持 MP3、FLAC、WAV、OGG、M4A、AAC 等常见格式，单个文件大小上限 100MB。",
    titleLabel: "标题",
    titlePlaceholder: "如果留空，将使用文件名或自动解析结果",
    artistLabel: "演唱者",
    optionalPlaceholder: "选填",
    albumLabel: "专辑",
    albumArtistLabel: "专辑演唱者",
    genreLabel: "流派",
    composerLabel: "作曲者",
    yearLabel: "年份",
    yearPlaceholder: "选填，示例：2025",
    trackNumberLabel: "曲目序号",
    discNumberLabel: "唱片序号",
    coverLabel: "封面图片地址",
    coverPlaceholder: "选填，可填写公开的图片链接",
    uploadButton: "上传歌曲",
    uploadingLabel: "上传中...",
    resetButton: "重置表单",
    successPrefix: "《",
    successSuffix: "》已添加到 ",
    successFallbackLibrary: "所选曲库",
    successDurationPrefix: "播放时长：",
    successDurationFallback: "未知",
    successBitratePrefix: "比特率：",
    successBitrateFallback: "未知",
    durationUnit: " 秒",
    bitrateUnit: " kbps",
  },
} as const;

export type ErrorMessageKey = keyof typeof ERROR_MESSAGES;
export type UiText = typeof UI_TEXT;