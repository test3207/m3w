  ...
  const ensurePreloadedTrack = useCallback(
    async (track: Track): Promise<PreloadResult> => {
      ...
      const preloadPromise: Promise<PreloadResult> = (async () => {
        try {
          const response = await fetch(track.audioUrl, { credentials: 'same-origin' });
          if (!response.ok) {
            throw new Error(`Failed to preload track ${track.id}`);
          }
          ...
          logger.error({ msg: 'Failed to preload track', error });
          return null;
        } finally {
